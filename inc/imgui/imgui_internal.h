...
struct ImGuiWindowTempData;  // Temporary storage for one, that's the data which in theory we could
                             // ditch at the end of the frame
...
typedef int
            ImGuiItemStatusFlags;  // flags: storage for DC.LastItemXXX        // enum ImGuiItemStatusFlags_
typedef int ImGuiNavHighlightFlags;  // flags: for RenderNavHighlight()          // enum
                                     // ImGuiNavHighlightFlags_
typedef int ImGuiNavDirSourceFlags;  // flags: for GetNavInputAmount2d()         // enum
                                     // ImGuiNavDirSourceFlags_
typedef int
    ImGuiNavMoveFlags;  // flags: for navigation requests           // enum ImGuiNavMoveFlags_
typedef int
            ImGuiSeparatorFlags;  // flags: for Separator() - internal        // enum ImGuiSeparatorFlags_
...
IMGUI_API int ImTextStrFromUtf8(ImWchar*     buf,
                                ...
                                = NULL);  // return input UTF-8 bytes count
...
enum ImGuiButtonFlags_
{
    ...
    = 1 << 9,  // vertically align button to match text baseline - ButtonEx() only // FIXME: Should
...
enum ImGuiNavMoveFlags_
{
    ...
    = 1 << 5  // Store alternate result in NavMoveResultLocalVisibleSet that only comprise elements
...
struct IMGUI_API ImGuiTextEditState
{
    ...
    ImVector<ImWchar> Text;  // edit buffer, we need to persist but can't guarantee the persistence
...
struct ImGuiContext
{
    ...
    ImGuiWindow*
        NavWindowingTarget;  // When selecting a window (holding Menu+FocusPrev/Next, or equivalent
                             // of CTRL-TAB) this window is temporarily displayed front-most.
    float NavWindowingHighlightTimer;
    float NavWindowingHighlightAlpha;
    bool  NavWindowingToggleLayer;
    int   NavLayer;  // Layer we are navigating on. For now the system is hard-coded for 0=main
    ...
    ImVec2         PlatformImePos,
        PlatformImeLastPos;  // Cursor position request & last passed to the OS Input Method Editor
...
enum ImGuiItemFlags_
{
    ...
    = 1
      << 2,  // false    // FIXME-WIP: Disable interactions but doesn't affect visuals. Should be:
             // grey out and disable interactions with widgets that affect data + view widgets (WIP)
...
struct IMGUI_API ImGuiWindowTempData
{
    ...
    float IndentX;  // Indentation / start position from left of window (increased by
                    // TreePush/TreePop, etc.)
    float GroupOffsetX;
    float ColumnsOffsetX;  // Offset to the current column (if ColumnsCurrent > 0). FIXME: This and
...
struct IMGUI_API ImGuiWindow
{
    ...
    ImVec2
        SizeFullAtLastBegin;  // Copy of SizeFull at the end of Begin. This is the reference value
                              // we'll use on the next frame to decide if we need scrollbars.
    ...
    int  BeginOrderWithinParent;  // Order within immediate parent window, if we are a child window.
                                  // Otherwise 0.
    int BeginOrderWithinContext;  // Order within entire imgui context. This is mostly used for
                                  // debugging submission order related issues.
    int BeginCount;   // Number of Begin() during the current frame (generally 0 or 1, 1+ if
    ...
    ImGuiWindow*
                 RootWindowForTabbing;  // Point to ourself or first ancestor which can be CTRL-Tabbed into.
...
IMGUI_API void
VerticalSeparator();  // Vertical separator, for menu bars (use current line height). not exposed
                      // because it is misleading what it doesn't have an effect on regular layout.
...
// SIGNATURE AND BEHAVIOR WILL CHANGE, THEY NEED TO BE REFACTORED INTO SOMETHING DECENT.

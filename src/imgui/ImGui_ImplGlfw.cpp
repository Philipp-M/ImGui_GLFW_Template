...
//  2018-07-07: Misc: use C++ namespaces and reformat to Allman Style with clang-format
//  2018-06-29: Inputs: Added support for the ImGuiMouseCursor_Hand cursor.
//  2018-06-08: Misc: Extracted imgui_impl_glfw.cpp/.h away from the old combined GLFW+OpenGL/Vulkan
//  examples. 2018-03-20: Misc: Setup io.BackendFlags ImGuiBackendFlags_HasMouseCursors flag + honor
//  ImGuiConfigFlags_NoMouseCursorChange flag. 2018-02-20: Inputs: Added support for mouse cursors
//  (ImGui::GetMouseCursor() value, passed to glfwSetCursor()). 2018-02-06: Misc: Removed call to
...
//  navigation and ImGuiConfigFlags_NavMoveMouse is set). 2018-01-20: Inputs: Added Horizontal Mouse
//  Wheel support. 2018-01-18: Inputs: Added mapping for ImGuiKey_Insert. 2017-08-25: Inputs:
...
bool InitForOpenGL(::GLFWwindow* window, bool install_callbacks)
{
    return Init(window, install_callbacks, GlfwClientApi_OpenGL);
}

bool InitForVulkan(::GLFWwindow* window, bool install_callbacks)
{
...
{
    ImGuiIO& io = ImGui::GetIO();
    IM_ASSERT(io.Fonts->IsBuilt());  // Font atlas needs to be built, call renderer _NewFrame()
    ...
    if (io.ConfigFlags & ImGuiConfigFlags_NavEnableGamepad)
    {
        ...
        auto mapAnalog = [&](ImGuiNavInput_ navNo, int axisNo, float v0, float v1) {
            float v = (axes_count > axisNo) ? axes[axisNo] : v0;
            v       = (v - v0) / (v1 - v0);
            if (v > 1.0f)
                v = 1.0f;
            if (io.NavInputs[navNo] < v)
                io.NavInputs[navNo] = v;
        };

        auto mapButton = [&](ImGuiNavInput_ navNo, int buttonNo) {
            if (buttons_count > buttonNo && buttons[buttonNo] == GLFW_PRESS)
                io.NavInputs[navNo] = 1.0f;
        };

        mapButton(ImGuiNavInput_Activate, 0);    // Cross / A
        mapButton(ImGuiNavInput_Cancel, 1);      // Circle / B
        mapButton(ImGuiNavInput_Menu, 2);        // Square / X
        mapButton(ImGuiNavInput_Input, 3);       // Triangle / Y
        mapButton(ImGuiNavInput_DpadLeft, 13);   // D-Pad Left
        mapButton(ImGuiNavInput_DpadRight, 11);  // D-Pad Right
        mapButton(ImGuiNavInput_DpadUp, 10);     // D-Pad Up
        mapButton(ImGuiNavInput_DpadDown, 12);   // D-Pad Down
        mapButton(ImGuiNavInput_FocusPrev, 4);   // L1 / LB
        mapButton(ImGuiNavInput_FocusNext, 5);   // R1 / RB
        mapButton(ImGuiNavInput_TweakSlow, 4);   // L1 / LB
        mapButton(ImGuiNavInput_TweakFast, 5);   // R1 / RB
        mapAnalog(ImGuiNavInput_LStickLeft, 0, -0.3f, -0.9f);
        mapAnalog(ImGuiNavInput_LStickRight, 0, +0.3f, +0.9f);
        mapAnalog(ImGuiNavInput_LStickUp, 1, +0.3f, +0.9f);
        mapAnalog(ImGuiNavInput_LStickDown, 1, -0.3f, -0.9f);

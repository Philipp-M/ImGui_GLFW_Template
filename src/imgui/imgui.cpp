...
/*
 ...
 - FREQUENTLY ASKED QUESTIONS (FAQ), TIPS
   ...
   - How can I have multiple widgets with the same label or without a label? A primer on labels and
 the ID Stack.
 ...
 there are no construction or destruction steps, less data retention on your side, less state
 duplication, less state synchronization, less bugs.
 ...
 - You can later customize the imconfig.h file to tweak some compilation time behavior, such as
 ...
 ImGui const ImDrawIdx* idx_buffer = cmd_list->IdxBuffer.Data;   // index buffer generated by ImGui
          for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++)
          {
             ...
             else
             {
                 ...
                 // The vast majority of draw calls with use the imgui texture atlas, which value
 you have set yourself during initialization. MyEngineBindTexture(pcmd->TextureId);
 ...
 future, always subtract draw_data->DisplayPos from
 ...
 (int)(pcmd->ClipRect.z - pos.x), (int)(pcmd->ClipRect.w - pos.y));
                 ...
                 // By default the indices ImDrawIdx are 16-bits, you can change them to 32-bits if
 your engine doesn't support 16-bits indices. MyEngineDrawIndexedTriangles(pcmd->ElemCount,
 sizeof(ImDrawIdx) == 2 ? GL_UNSIGNED_SHORT : GL_UNSIGNED_INT, idx_buffer, vtx_buffer);
             }
 ...
 'io.WantTextInput' flags are updated. They tell you if ImGui intends to use your inputs. When a
 flag is set you want to hide the corresponding inputs from the rest of your application. In both
 cases you need to pass on the inputs to imgui. Read the FAQ below for more information about those
 flags.
 - Please read the FAQ above. Amusingly, it is called a FAQ because people frequently have the same
 issues!

 USING GAMEPAD/KEYBOARD NAVIGATION CONTROLS [BETA]

 - The gamepad/keyboard navigation is in Beta. Ask questions and report issues at
 ...
 - 2018/07/06 (1.63) - removed per-window ImGuiWindowFlags_ResizeFromAnySide beta flag in favor of a
 global io.OptResizeWindowsFromEdges to enable the feature.
 ...
 cmd_lists_count) new: ImGui_XXXX_RenderDrawLists(ImDrawData* draw_data). argument:   'cmd_lists'
 ...
 <..Upload texture to GPU>; io.Fonts->TexId = YourTextureIdentifier; you now more flexibility to
 load multiple TTF fonts and manage the texture buffer for internal needs. it is now recommended
 ...
 imgui needs to detect that you clicked in the void to unfocus its windows. Note: The
 ...
 A: The most convenient and practical way is to merge an icon font such as FontAwesome inside you
    main font. Then you can refer to icons within your strings. Read 'How can I load multiple
 fonts?' and the file 'misc/fonts/README.txt' for instructions and useful header files.
 ...
 only build the characters the game need) ImVector<ImWchar> ranges; ImFontAtlas::GlyphRangesBuilder
 ...
 A: - You can create a dummy window. Call SetNextWindowBgAlpha(0.0f), call Begin() with
 NoTitleBar|NoResize|NoMove|NoScrollbar|NoSavedSettings|NoInputs flags. Then you can retrieve the
 ImDrawList* via GetWindowDrawList() and draw to it in any way you like.
 ...
 ImGui::GetDrawListSharedData(), or create your own ImDrawListSharedData.
 ...
 Q: How can I help?
 A: - If you are experienced with Dear ImGui and C++, look at the github issues, or TODO.txt and see
 how you want/can help!
    - Convince your company to fund development time! Individual users: you can also become a Patron
 ...
 (github.com/ocornut/imgui/issues/1269). Visuals are ideal as they inspire other programmers. But
...
#if __GNUC__ >= 8
#pragma GCC diagnostic ignored                                                                     \
    "-Wclass-memaccess"  // warning: 'memset/memcpy' clearing/writing an object of type 'xxxx' with
                         // no trivial copy-assignment; use assignment or value-initialization
                         // instead
#endif
#endif
...
static void DataTypeApplyOp(
    ImGuiDataType data_type, int op, void* output, void* arg_1, const void* arg_2);
...
static bool DragBehaviorT(ImGuiDataType data_type,
                          TYPE*         v,
                          float         v_speed,
                          const TYPE    v_min,
                          const TYPE    v_max,
...
template<typename TYPE, typename SIGNEDTYPE, typename FLOATTYPE>
static bool SliderBehaviorT(const ImRect&    bb,
                            ImGuiID          id,
                            ImGuiDataType    data_type,
                            TYPE*            v,
                            const TYPE       v_min,
...
// CreateContext() will automatically set this pointer if it is NULL. Change to a different context
// by calling ImGui::SetCurrentContext(). If you use DLL hotreloading you might need to call
...
ImGuiStyle::ImGuiStyle()
{
    ...
    DisplayWindowPadding
        = ImVec2(20, 20);  // Window positions are clamped to be visible within the display area by
                           // at least this amount. Only covers regular windows.
...
static void SetCursorPosYAndSetupDummyPrevLine(float pos_y, float line_height)
{
    ...
    window->DC.CursorPosPrevLine.y
        = window->DC.CursorPos.y
          - line_height;  // Setting those fields so that SetScrollHere() can properly function
                          // after the end of our clipper usage.
    window->DC.PrevLineHeight
        = (line_height
           - GImGui->Style.ItemSpacing
                 .y);  // If we end up needing more accurate data (to e.g. use SameLine) we may as
                       // well make the clipper have a fourth step to let user process and display
                       // the last item in their list.
...
bool ImGuiListClipper::Step()
{
    ...
    if (StepNo == 2)  // Step 2: dummy step only required if an explicit items_height was passed to
                      // constructor or Begin() and user still call Step(). Does nothing and switch
                      // to Step 3.
    {
...
void ImGui::SetHoveredID(ImGuiID id)
{
    ...
    g.HoveredIdTimer
        = (id != 0 && g.HoveredIdPreviousFrame == id) ? (g.HoveredIdTimer + g.IO.DeltaTime) : 0.0f;
}
...
static void NavUpdateWindowingHighlightWindow(int focus_change_dir)
{
    ...
    ImGuiWindow* window_target
        = FindWindowNavigable(i_current + focus_change_dir, -INT_MAX, focus_change_dir);
    if (!window_target)
        window_target = FindWindowNavigable(
            (focus_change_dir < 0) ? (g.Windows.Size - 1) : 0, i_current, focus_change_dir);
...
static void ImGui::NavUpdateWindowing()
{
    ...
    if (start_windowing_with_gamepad || start_windowing_with_keyboard)
        if (ImGuiWindow* window
            = g.NavWindow ? g.NavWindow : FindWindowNavigable(g.Windows.Size - 1, -INT_MAX, -1))
        {
    ...
    if (apply_focus_window
        && (g.NavWindow == NULL || apply_focus_window != g.NavWindow->RootWindowForTabbing))
    {
        g.NavDisableHighlight  = false;
        g.NavDisableMouseHover = true;
    ...
    if (apply_toggle_layer && g.NavWindow)
    {
        ...
        while ((new_nav_window->DC.NavLayerActiveMask & (1 << 1)) == 0
               ...
               && (new_nav_window->Flags & (ImGuiWindowFlags_Popup | ImGuiWindowFlags_ChildMenu))
                      == 0)
...
#undef NAV_MAP_KEY
    ...
    if (g.NavMoveRequest && (g.NavMoveResultLocal.ID != 0 || g.NavMoveResultOther.ID != 0))
    {
        ...
        // PageUp/PageDown behavior first jumps to the bottom/top mostly visible item, _otherwise_
        // use the result from the previous/next page.
        if (g.NavMoveRequestFlags & ImGuiNavMoveFlags_AlsoScoreVisibleSet)
        ...
        if (g.NavLayer == 0)
        {
            ...
            // Estimate upcoming scroll so we can offset our result position so mouse position can
            // be applied immediately after in NavUpdate()
            ImVec2 next_scroll  = CalcNextScrollFromScrollTargetAndClamp(result->Window, false);
            ImVec2 delta_scroll = result->Window->Scroll - next_scroll;
            result->RectRel.Translate(delta_scroll);

            // Also scroll parent window to keep us into view if necessary (we could/should
            // technically recurse back the whole the parent hierarchy).
    ...
    if (g.NavMoveRequestForward == ImGuiNavForward_None)
    {
            ...
            && !(g.NavWindow->Flags & ImGuiWindowFlags_NoNavInputs))
        {
            ...
            if ((allowed_dir_flags & (1 << ImGuiDir_Up))
                && IsNavInputPressedAnyOfTwo(
                       ImGuiNavInput_DpadUp, ImGuiNavInput_KeyUp_, ImGuiInputReadMode_Repeat))
...
static void ImGui::UpdateMouseInputs()
{
    ...
    // If mouse just appeared or disappeared (usually denoted by -FLT_MAX component, but in reality
    // we test for -256000.0f) we cancel out movement in MouseDelta
    if (IsMousePosValid(&g.IO.MousePos) && IsMousePosValid(&g.IO.MousePosPrev))
...
void ImGui::NewFrame()
{
    ...
    // Perform simple check for required key mapping (we intentionally do NOT check all keys to not
    ...
    // The beta io.OptResizeWindowsFromEdges option requires back-end to honor mouse cursor changes
    // and set the ImGuiBackendFlags_HasMouseCursors flag accordingly.
    ...
    g.IO.Framerate
        = (g.FramerateSecPerFrameAccum > 0.0f)
              ? (1.0f
                 / (g.FramerateSecPerFrameAccum / (float) IM_ARRAYSIZE(g.FramerateSecPerFrame)))
              : FLT_MAX;
    ...
    {
        // If a child window has the ImGuiWindowFlags_NoScrollWithMouse flag, we give a chance to
        // scroll its parent (unless either ImGuiWindowFlags_NoInputs or
        // ImGuiWindowFlags_NoScrollbar are also set).
    ...
    // We don't use "Debug" to avoid colliding with user trying to create a "Debug" window with
    // custom flags.
...
static void AddWindowToDrawData(ImVector<ImDrawList*>* out_render_list, ImGuiWindow* window)
{
    ...
    for (int i = 0; i < window->DC.ChildWindows.Size; i++)
    {
        ImGuiWindow* child = window->DC.ChildWindows[i];
        if (child->Active
            && child->HiddenFrames == 0)  // clipped children may have been marked not active
...
void ImGui::EndFrame()
{
    ...
    IM_ASSERT(g.CurrentWindowStack.Size == 1);  // Mismatched Begin()/End() calls, did you forget to
                                                // call end on g.CurrentWindow->Name?
    ...
    if (g.ActiveId == 0 && g.HoveredId == 0)
    {
        if (!g.NavWindow || !g.NavWindow->Appearing)  // Unless we just made a window/popup appear
        {
            ...
            if (g.IO.MouseClicked[0])
            {
                ...
                {
                    // Set ActiveId even if the _NoMove flag is set, without it dragging away from a
                    // window with _NoMove would activate hover on other windows.
...
void ImGui::Render()
{
        ...
        && g.IO.Fonts->GetMouseCursorTexData(g.MouseCursor, &offset, &size, &uv[0], &uv[2]))
    {
        ...
        g.OverlayDrawList.AddImage(tex_id,
                                   pos + ImVec2(1, 0) * sc,
                                   pos + ImVec2(1, 0) * sc + size * sc,
                                   uv[2],
                                   uv[3],
                                   IM_COL32(0, 0, 0, 48));  // Shadow
        g.OverlayDrawList.AddImage(tex_id,
                                   pos + ImVec2(2, 0) * sc,
                                   pos + ImVec2(2, 0) * sc + size * sc,
                                   uv[2],
                                   uv[3],
                                   IM_COL32(0, 0, 0, 48));  // Shadow
        g.OverlayDrawList.AddImage(
            tex_id, pos, pos + size * sc, uv[2], uv[3], IM_COL32(0, 0, 0, 255));  // Black border
        g.OverlayDrawList.AddImage(tex_id,
                                   pos,
                                   pos + size * sc,
                                   uv[0],
    ...
    for (;;)
    {
        ...
        if (line_end != NULL && !(is_last_line && (line_end - text_remaining) == 0))
        {
            ...
            if (log_new_line || !is_first_line)
                ImGui::LogText(
                    IM_NEWLINE "%*s%.*s", tree_depth * 4, "", char_count, text_remaining);
            else
...
bool ImGui::IsItemDeactivatedAfterChange()
{
    ...
    return IsItemDeactivated()
           && (g.ActiveIdPreviousFrameValueChanged || (g.ActiveId == 0 && g.ActiveIdValueChanged));
}
...
void ImGui::ClosePopupsOverWindow(ImGuiWindow* ref_window)
{
    ...
    if (n < g.OpenPopupStack.Size)  // This test is not required but it allows to set a convenient
                                    // breakpoint on the block below
        ClosePopupToLevel(n);
}
...
static bool ImGui::BeginChildEx(
    const char* name, ImGuiID id, const ImVec2& size_arg, bool border, ImGuiWindowFlags flags)
{
    ImGuiContext& g             = *GImGui;
    ImGuiWindow*  parent_window = g.CurrentWindow;
...
// we ignore it.
static ImVec2 FindBestWindowPosForPopupEx(const ImVec2&            ref_pos,
                                          const ImVec2&            size,
                                          ImGuiDir*                last_dir,
                                      ...
                                      ImU32         resize_grip_col[4])
{
    ...
    for (int border_n = 0; border_n < resize_border_count; border_n++)
    {
        const float BORDER_SIZE
            = 5.0f;  // FIXME: Only works _inside_ window because of HoveredWindow check.
...
bool ImGui::Begin(const char* name, bool* p_open, ImGuiWindowFlags flags)
{
    ...
    {
        ImVec2 size_on_first_use = (g.NextWindowData.SizeCond != 0)
    ...
    if (first_begin_of_the_frame)
    {
        ...
        window->RootWindow = window->RootWindowForTitleBarHighlight = window->RootWindowForTabbing
        ...
        if (parent_window && !(flags & ImGuiWindowFlags_Modal)
            ...
            window->RootWindowForTitleBarHighlight = window->RootWindowForTabbing
                = parent_window->RootWindowForTitleBarHighlight;  // Same value in master branch,
                                                                  // will differ for docking
        ...
        if (!(flags & ImGuiWindowFlags_ChildWindow))
        {
            if (!window_pos_set_by_api && window->AutoFitFramesX <= 0 && window->AutoFitFramesY <= 0
                ...
                && g.IO.DisplaySize.y > 0.0f)  // Ignore zero-sized display explicitly to avoid
        ...
        if (window->Collapsed)
        {
            ...
            RenderFrame(
                title_bar_rect.Min, title_bar_rect.Max, title_bar_col, true, window_rounding);
        ...
        else
        {
            ...
            if (flags & ImGuiWindowFlags_MenuBar)
            {
                ...
                menu_bar_rect.ClipWith(
                    window
                        ->Rect());  // Soft clipping, in particular child window don't have minimum
            ...
            if (!(flags & ImGuiWindowFlags_NoResize))
            {
                for (int resize_grip_n = 0; resize_grip_n < resize_grip_count; resize_grip_n++)
                {
                    ...
                    window->DrawList->PathLineTo(
                        corner
                        + grip.InnerDir
                              * ((resize_grip_n & 1) ? ImVec2(window_border_size, grip_draw_size)
                                                     : ImVec2(grip_draw_size, window_border_size)));
                    window->DrawList->PathLineTo(
                        corner
                        + grip.InnerDir
                              * ((resize_grip_n & 1) ? ImVec2(grip_draw_size, window_border_size)
        ...
        if (!(flags & ImGuiWindowFlags_NoTitleBar))
        {
            ...
            if (!(flags & ImGuiWindowFlags_NoCollapse))
            {
                ...
                ImRect  bb(window->Pos + style.FramePadding + ImVec2(1, 1),
                          window->Pos + style.FramePadding + ImVec2(g.FontSize, g.FontSize)
                              - ImVec2(1, 1));
                ItemAdd(bb, id);
                if (ButtonBehavior(bb, id, NULL, NULL))
                    window->CollapseToggleWanted = true;  // Defer collapsing to next frame as we
                                                          // are too far in the Begin() function
                RenderNavHighlight(bb, id);
                RenderArrow(window->Pos + style.FramePadding,
            ...
            // this is too much code for what it does.)
    ...
    if (flags & ImGuiWindowFlags_ChildWindow)
    {
        ...
        if (!(flags & ImGuiWindowFlags_AlwaysAutoResize) && window->AutoFitFramesX <= 0
            ...
            window->Collapsed
                |= (window->OuterRectClipped.Min.x >= window->OuterRectClipped.Max.x
...
void ImGui::Scrollbar(ImGuiLayoutType direction)
{
    ...
    IM_ASSERT(ImMax(win_size_contents_v, win_size_avail_v)
              > 0.0f);  // Adding this assert to check if the ImMax(XXX,1.0f) is still needed.
...
static void SetWindowPos(ImGuiWindow* window, const ImVec2& pos, ImGuiCond cond)
{
    ...
    window->DC.CursorPos
        += (window->Pos
            - old_pos);  // As we happen to move the window while it is being appended to (which is
...
void ImGui::SetNextWindowContentSize(const ImVec2& size)
{
    ...
    g.NextWindowData.ContentSizeVal
        = size;  // In Begin() we will add the size of window decorations (title bar, menu etc.) to
                 // that to form a SizeContents value.
...
void ImGui::SetScrollHere(float center_y_ratio)
{
    ...
    target_y += (window->DC.PrevLineHeight * center_y_ratio)
...
void ImGui::AlignTextToFramePadding()
{
    ...
    ImGuiContext& g = *GImGui;
    window->DC.CurrentLineHeight
        = ImMax(window->DC.CurrentLineHeight, g.FontSize + g.Style.FramePadding.y * 2);
    ...
    const ImRect& bb, ImGuiID id, bool* out_hovered, bool* out_held, ImGuiButtonFlags flags)
{
    ...
    if ((flags
         & (ImGuiButtonFlags_PressedOnClickRelease | ImGuiButtonFlags_PressedOnClick
                             ...
                             const char*        label_end)
{
    ...
    const float text_width
        = g.FontSize
          + (label_size.x > 0.0f ? label_size.x + padding.x * 2 : 0.0f);  // Include collapser
                                           ...
                                           const char*   format)
{
    ...
    if (value_changed)
        return DataTypeApplyOpFromText(
            data_buf, g.InputTextState.InitialText.begin(), data_type, data_ptr, NULL);
    return false;
}
...
template<typename TYPE, typename SIGNEDTYPE, typename FLOATTYPE>
static bool ImGui::SliderBehaviorT(const ImRect&    bb,
                                   ImGuiID          id,
                                   ImGuiDataType    data_type,
                                   TYPE*            v,
                                   const TYPE       v_min,
                                   ...
                                   ImGuiSliderFlags flags)
{
    ...
    if (g.ActiveId == id)
    {
        ...
        {
            const ImVec2 delta2 = GetNavInputAmount2d(ImGuiNavDirSourceFlags_Keyboard
            ...
            {
                clicked_t = SliderBehaviorCalcRatioFromValue<TYPE, FLOATTYPE>(
                ...
                if ((clicked_t >= 1.0f && delta > 0.0f)
                    || (clicked_t <= 0.0f
                        && delta < 0.0f))  // This is to avoid applying the saturation when already
                                           // past the limits
                    set_new_value = false;
                else
    ...
    // Draw
    float grab_t = SliderBehaviorCalcRatioFromValue<TYPE, FLOATTYPE>(
        data_type, *v, v_min, v_max, power, linear_zero_pos);
    if (!is_horizontal)
        grab_t = 1.0f - grab_t;
    ...
    ImRect      grab_bb;
    if (is_horizontal)
        grab_bb = ImRect(grab_pos - grab_sz * 0.5f,
                         bb.Min.y + grab_padding,
                         grab_pos + grab_sz * 0.5f,
                         bb.Max.y - grab_padding);
    else
        grab_bb = ImRect(bb.Min.x + grab_padding,
                         grab_pos - grab_sz * 0.5f,
                         ...
                         float         power)
{
    ...
    const char* value_buf_end
        = value_buf
          + DataTypeFormatString(value_buf, IM_ARRAYSIZE(value_buf), data_type, v, format);
    RenderTextClipped(
                          ...
                          float         power)
{
    ...
    const bool value_changed = SliderBehavior(
        frame_bb, id, data_type, v, v_min, v_max, format, power, ImGuiSliderFlags_Vertical);
    if (value_changed)
        MarkItemValueChanged(id);
...
// with Nav controls)
template<typename TYPE, typename SIGNEDTYPE, typename FLOATTYPE>
static bool ImGui::DragBehaviorT(ImGuiDataType data_type,
                                 TYPE*         v,
                                 float         v_speed,
                                 const TYPE    v_min,
                                 const TYPE    v_max,
                                 ...
                                 float         power)
{
    ...
    if (is_power)
    {
        ...
        v_cur
            = v_min + (TYPE) ImPow(ImSaturate((float) v_new_norm_curved), power) * (v_max - v_min);
                   ...
                   ImVec2      graph_size)
{
    ...
    if (values_count > 0)
    {
        ...
        float  t0  = 0.0f;
        ImVec2 tp0 = ImVec2(
            t0, 1.0f - ImSaturate((v0 - scale_min) * inv_scale));  // Point in the normalized space
...
void ImGui::ProgressBar(float fraction, const ImVec2& size_arg, const char* overlay)
{
    ...
    ImRect bb(
        pos,
        pos + CalcItemSize(size_arg, CalcItemWidth(), g.FontSize + style.FramePadding.y * 2.0f));
    ItemSize(bb, style.FramePadding.y);
    if (!ItemAdd(bb, 0))
        return;
    ...
    const ImRect check_bb(
        ...
        window->DC.CursorPos
            + ImVec2(label_size.y + style.FramePadding.y * 2,
                     label_size.y
                         + style.FramePadding.y * 2));  // We want a square shape to we use Y twice
                        ...
                        void*                 user_data)
{
    ...
    if (focus_requested || user_clicked || user_scrolled || user_nav_input_start)
    {
        if (g.ActiveId != id)
        {
            ...
            edit_state.CurLenA
                = (int) (buf_end
                         - buf);  // We can't get the result from ImFormatString() above because it
    ...
    if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id)
    {
        ...
        const bool is_startend_key_down = is_osx && io.KeySuper && !io.KeyCtrl
                                          && !io.KeyAlt;  // OS X style: Line/Text Start and End
...
// NB: format here must be a simple "%xx" format string with no prefix/suffix (unlike the
// Drag/Slider functions "format" argument)
                        ...
                        ImGuiInputTextFlags extra_flags)
{
    ...
    bool value_changed = false;
    if ((extra_flags & (ImGuiInputTextFlags_CharsHexadecimal | ImGuiInputTextFlags_CharsScientific))
        == 0)
    ...
    if (step != NULL)
    {
        ...
        if (InputText(
                "", buf, IM_ARRAYSIZE(buf), extra_flags))  // PushId(label) + "" gives us the
...
bool ImGui::InputInt(
    const char* label, int* v, int step, int step_fast, ImGuiInputTextFlags extra_flags)
{
...
// But you need to make sure the ID is unique, e.g. enclose calls in PushID/PopID or use
// ##unique_id.
...
{
    // Size default to hold ~7 items. Fractional number of items helps seeing that we can scroll
    // down/up without looking at scrollbar. We don't add +0.40f if items_count <= height_in_items.
    // It is slightly dodgy, because it means a dynamic list of items will make the widget resize
    // occasionally when it crosses that size. I am expecting that someone will come and complain
    // about this behavior in a remote future, then we can advise on a better solution.
...
bool ImGui::MenuItem(const char* label, const char* shortcut, bool selected, bool enabled)
{
    ...
    else
    {
        ...
        if (shortcut_size.x > 0.0f)
        {
            PushStyleColor(ImGuiCol_Text, g.Style.Colors[ImGuiCol_TextDisabled]);
            RenderText(
                pos + ImVec2(window->MenuColumns.Pos[1] + extra_w, 0.0f), shortcut, NULL, false);
            PopStyleColor();
        }
        if (selected)
            RenderCheckMark(pos
                                      ...
                                      // (ImGuiWindowFlags_Popup|ImGuiWindowFlags_ChildMenu))
    {
            ...
            && !(window->Flags & ImGuiWindowFlags_MenuBar))
        {
            if (ImGuiWindow* next_window = g.OpenPopupStack[g.CurrentPopupStack.Size].Window)
            {
                ...
                float extra = ImClamp(
                    ImFabs(ta.x - tb.x) * 0.30f, 5.0f, 30.0f);  // add a bit of extra slack.
                ta.x += (window->Pos.x < next_window->Pos.x) ? -0.5f
                                                             : +0.5f;  // to avoid numerical issues
                tb.y = ta.y
                       + ImMax((tb.y - extra) - ta.y,
                               -100.0f);  // triangle is maximum 200 high to limit the slope and the
                                          // bias toward large sub-menus // FIXME: Multiply by
                                          // fb_scale?
...
void ImGui::EndMenu()
{
    ...
    // However, it means that with the current code, a BeginMenu() from outside another menu or a
    ...
    if (((col & IM_COL32_A_MASK) >> IM_COL32_A_SHIFT) < 0xFF)
    {
        ...
        for (float y = p_min.y + grid_off.y; y < p_max.y; y += grid_step, yi++)
        {
                 ...
                 x += grid_step * 2.0f)
            {
                ...
                window->DrawList->AddRectFilled(ImVec2(x1, y1),
                                                ImVec2(x2, y2),
                                                col_bg2,
                        ...
                        ImVec2              size)
{
    ...
    if ((flags & ImGuiColorEditFlags_AlphaPreviewHalf) && col.w < 1.0f)
    {
        ...
        RenderColorRectWithAlphaCheckerboard(ImVec2(bb_inner.Min.x + grid_step, bb_inner.Min.y),
                                             bb_inner.Max,
                                             GetColorU32(col),
                                             grid_step,
    ...
    if (g.ActiveId == id && !(flags & ImGuiColorEditFlags_NoDragDrop) && BeginDragDropSource())
    {
        if (flags & ImGuiColorEditFlags_NoAlpha)
            SetDragDropPayload(
                IMGUI_PAYLOAD_TYPE_COLOR_3F, &col, sizeof(float) * 3, ImGuiCond_Once);
        else
            SetDragDropPayload(
                IMGUI_PAYLOAD_TYPE_COLOR_4F, &col, sizeof(float) * 4, ImGuiCond_Once);
...
static void ColorPickerOptionsPopup(ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    if (allow_opt_alpha_bar)
    {
        if (allow_opt_picker)
            ImGui::Separator();
        ImGui::CheckboxFlags(
            "Alpha Bar", (unsigned int*) &g.ColorEditOptions, ImGuiColorEditFlags_AlphaBar);
    }
    ImGui::EndPopup();
}
        ...
        && (flags & ImGuiColorEditFlags_NoInputs) == 0)
    {
        ...
        for (int n = 0; n < components; n++)
        {
            ...
            if (flags & ImGuiColorEditFlags_Float)
                value_changed = value_changed_as_float = value_changed
                                                         | DragFloat(ids[n],
                                                                     &f[n],
                                                                     1.0f / 255.0f,
                                                                     0.0f,
...
// 'pos' is position of the arrow tip. half_sz.x is length from base to tip. half_sz.y is length on
// each side.
...
void ImGui::EndGroup()
{
    ...
    if (group_data.AdvanceCursor)
    {
        window->DC.CurrentLineTextBaseOffset = ImMax(
            ...
            group_data.BackupCurrentLineTextBaseOffset);  // FIXME: Incorrect, we should grab the
                                                          // base offset from the *first line* of
                                                          // the group but it is hard to obtain now.
    ...
    // LastItemIsActive', but put a little more burden on individual widgets. (and if you grep for
...
void ImGui::NewLine()
{
    ...
    if (window->DC.CurrentLineHeight
        > 0.0f)  // In the event that we are on a line with items that is smaller that FontSize
...
void ImGui::NextColumn()
{
    ...
    if (++columns->Current < columns->Count)
    {
        ...
        window->DC.ColumnsOffsetX
            = GetColumnOffset(columns->Current) - window->DC.IndentX + g.Style.ItemSpacing.x;
...
void ImGui::BeginDragDropTooltip()
{
    ...
    // also clamped within the current viewport/monitor) In the context of a dragging tooltip we try
    // to reduce that offset and we enforce following the cursor. Whatever we do we want to call
    // SetNextWindowPos() to enforce a tooltip position and disable clipping the tooltip without our
    // display area, like regular tooltip do.
    ImGuiContext& g = *GImGui;
...
void ImGui::ShowMetricsWindow(bool* p_open)
{
    if (ImGui::Begin("ImGui Metrics", p_open))
    {
        ...
        struct Funcs
        {
            static void NodeDrawList(ImGuiWindow* window, ImDrawList* draw_list, const char* label)
            {
                ...
                ImDrawList* overlay_draw_list
                    = GetOverlayDrawList();  // Render additional visuals into the top-most draw
                                             // list
            ...
            static void NodeWindow(ImGuiWindow* window, const char* label)
            {
                ...
                ImGui::BulletText(
                    "Active: %d, WriteAccessed: %d", window->Active, window->WriteAccessed);
        ...
        if (ImGui::TreeNode("Internal state"))
        {
            ...
            ImGui::Text("ActiveId: 0x%08X/0x%08X (%.2f sec), AllowOverlap: %d, Source: %s",
                        g.ActiveId,

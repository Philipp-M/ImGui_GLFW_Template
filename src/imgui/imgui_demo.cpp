...
// Don't do it! Do NOT remove this file from your project! It is useful reference code that you and
// other users will want to refer to. Everything in this file will be stripped out by the linker if
// you don't call ImGui::ShowDemoWindow(). During development, you can call ImGui::ShowDemoWindow()
// in your code to learn about various features of ImGui. Have it wired in a debug menu! Removing
// this file from your project is hindering access to documentation for everyone in your team,
// likely leading you to poorer usage of the library. Note that you can #define
// IMGUI_DISABLE_DEMO_WINDOWS in imconfig.h for the same effect. If you want to link core ImGui in
// your final builds but not those demo windows, #define IMGUI_DISABLE_DEMO_WINDOWS in imconfig.h
// and those functions will be empty. In other situation, when you have ImGui available you probably
// want this to be available for reference and execution. Thank you, -Your beloved friend,
// imgui_demo.cpp (that you won't delete)

// Message to beginner C/C++ programmers about the meaning of the 'static' keyword: in this demo
// code, we frequently we use 'static' variables inside functions. A static variable persist across
// calls, so it is essentially like a global variable but declared inside the scope of the function.
// We do this as a way to gather code and data in the same place, just to make the demo code faster
// to read, faster to write, and use less code. It also happens to be a convenient way of storing
// simple UI related information as long as your function doesn't need to be reentrant or used in
// threads. This might be a pattern you occasionally want to use in your code, but most of the real
// data you would be editing is likely to be stored outside your functions.
...
void ImGui::ShowDemoWindow(bool* p_open)
{
    ...
    // We specify a default size in case there's no data in the .ini file. Typically this isn't
    // required! We only do it to make the Demo applications a little more welcoming.
    ...
    if (ImGui::CollapsingHeader("Widgets"))
    {
        if (ImGui::TreeNode("Basic"))
        {
            ...
            // Color buttons, demonstrate using PushID() to add unique identifier in the ID stack,
            // and changing style.
            for (int i = 0; i < 7; i++)
            {
                if (i > 0)
        ...
        if (ImGui::TreeNode("Trees"))
        {
            ...
            if (ImGui::TreeNode("Advanced, with Selectable nodes"))
            {
                ...
                static int selection_mask
                    = (1 << 2);  // Dumb representation of what may be user-side selection state.
                                 // You may carry selection state inside or outside your objects in
        ...
        if (ImGui::TreeNode("Text"))
        {
            ...
            if (ImGui::TreeNode("UTF-8 Text"))
            {
                ...
                // - From C++11 you can use the u8"my text" syntax to encode literal strings as
                // UTF-8
                // - For earlier compiler, you may be able to encode your sources as UTF-8 (e.g.
                // Visual Studio save your file as 'UTF-8 without signature')
                ...
                //   Instead we are encoding a few strings with hexadecimal constants. Don't do this
                //   in your application! Please use u8"text in any language" in your application!
                // Note that characters values are preserved even by InputText() if the font cannot
                // be displayed, so you can safely copy & paste garbled characters into another
                // application.
                ImGui::TextWrapped(
                ...
                // static char buf[32] = u8"NIHONGO"; // <- this is how you would write it with
                // C++11, using real kanjis
        ...
        if (ImGui::TreeNode("Images"))
        {
            ...
            // inside the demo code. Remember that ImTextureID is just storage for whatever you want
            // it to be, it is essentially a value that will be passed to the render function inside
            // the ImDrawCmd structure. If you use one of the default imgui_impl_XXXX.cpp renderer,
            // they all have comments at the top of their file to specify what they expect to be
            // stored in ImTextureID. (for example, the imgui_impl_dx11.cpp renderer expect a
            // 'ID3D11ShaderResourceView*' pointer. The imgui_impl_glfw_gl3.cpp renderer expect a
            // GLuint OpenGL texture identifier etc.) If you decided that ImTextureID =
            // MyEngineTexture*, then you can pass your MyEngineTexture* pointers to ImGui::Image(),
            // and gather width/height through your own functions, etc. Using ShowMetricsWindow() as
            // a "debugger" to inspect the draw data that are being passed to your render will help
            // you debug issues if you are confused about this. Consider using the lower-level
        ...
        if (ImGui::TreeNode("Combo"))
        {
            ...
            if (ImGui::CheckboxFlags("ImGuiComboFlags_NoArrowButton",
                ...
                flags &= ~ImGuiComboFlags_NoPreview;  // Clear the other flag, as we cannot combine
                                                      // both
            if (ImGui::CheckboxFlags(
                    "ImGuiComboFlags_NoPreview", (unsigned int*) &flags, ImGuiComboFlags_NoPreview))
                flags &= ~ImGuiComboFlags_NoArrowButton;  // Clear the other flag, as we cannot
                                                          // combine both
            ...
            if (ImGui::BeginCombo(
                    "combo 1", item_current, flags))  // The second parameter is the label previewed
                ...
                for (int n = 0; n < IM_ARRAYSIZE(items); n++)
                {
                    ...
                    if (is_selected)
                        ImGui::SetItemDefaultFocus();  // Set the initial focus when opening the
                                                       // combo (scrolling + for keyboard navigation
                                                       // support in the upcoming navigation branch)
                }
                ImGui::EndCombo();
            }
            ...
            ImGui::Combo(
                "combo 2 (one-liner)", &item_current_2, "aaaa\0bbbb\0cccc\0dddd\0eeee\0\0");
        ...
        if (ImGui::TreeNode("Selectables"))
        {
            ...
            // - The one taking "bool selected" as a read-only selection information. When
            // Selectable() has been clicked is returns true and you can alter selection state
            // accordingly.
            // - The one taking "bool* p_selected" as a read-write selection information (convenient
            // in some cases) The earlier is more flexible, as in real application your selection
            // may be stored in a different manner (in flags within objects, as an external list,
            // etc).
            if (ImGui::TreeNode("Basic"))
            {
            ...
            {
                // Using the Selectable() override that takes "bool* p_selected" parameter and
                // toggle your booleans automatically.
        ...
        if (ImGui::TreeNode("Plots Widgets"))
        {
            ...
            // Tip: If your float aren't contiguous but part of a structure, you can pass a pointer
            // to your first float and the sizeof() of your structure in the Stride parameter.
            ...
            // FIXME: This is rather awkward because current plot API only pass in indices. We
            // probably want an API passing floats and user provide sample rate/count.
            struct Funcs
            {
        ...
        if (ImGui::TreeNode("Color/Picker Widgets"))
        {
            ...
            ImGui::ColorEdit4(
                "MyColor##2f", (float*) &color, ImGuiColorEditFlags_Float | misc_flags);
            ...
            if (!alpha)
                flags |= ImGuiColorEditFlags_NoAlpha;  // This is by default if you call
                                                       // ColorPicker3() instead of ColorPicker4()
            if (alpha_bar)
        ...
        if (ImGui::TreeNode("Range Widgets"))
        {
            ...
            ImGui::DragFloatRange2(
                "range", &begin, &end, 0.25f, 0.0f, 100.0f, "Min: %.1f %%", "Max: %.1f %%");
        ...
        {
            // The DragScalar, InputScalar, SliderScalar functions allow manipulating most common
            // data types: signed/unsigned int/long long and float/double To avoid polluting the
            // public API with all possible combinations, we use the ImGuiDataType enum to pass the
            // type, and argument-by-values are turned into argument-by-address. This is the reason
            // the test code below creates local variables to hold "zero" "one" etc. for each types.
            // In practice, if you frequently use a given type that is not covered by the normal API
            // entry points, you may want to wrap it yourself inside a 1 line function which can
            // take typed values argument instead of void*, and then pass their address to the
            // generic function. For example:
            //   bool SliderU64(const char *label, u64* value, u64 min = 0, u64 max = 0, const char*
            //   format = "%lld") { return SliderScalar(label, ImGuiDataType_U64, value, &min, &max,
            //   format); }
            // Below are helper variables we can take the address of to work-around this:
            // Note that the SliderScalar function has a maximum usable range of half the natural
            // type maximum, hence the /2 below.
            ...
            const double f64_zero = 0., f64_one = 1., f64_lo_a = -1000000000000000,
                         f64_hi_a = +1000000000000000;
            ...
            ImGui::DragScalar(
                "drag float", ImGuiDataType_Float, &f32_v, 0.005f, &f32_zero, &f32_one, "%f", 1.0f);
        ...
        {
            // Display the value of IsItemHovered() and other common item state functions. Note that
            // the flags can be combined. (because BulletText is an item itself and that would
            // affect the output of IsItemHovered() we pass all state in a single call to simplify
            // the code).
    ...
    if (ImGui::CollapsingHeader("Layout"))
    {
        ...
        if (ImGui::TreeNode("Text Baseline Alignment"))
        {
            ...
            ImGui::AlignTextToFramePadding();  // If your line starts with text, call this to align
            ...
            ImGui::AlignTextToFramePadding();  // Vertically align text node a bit lower so it'll be
                                               // vertically centered with upcoming widget.
        ...
        if (ImGui::TreeNode("Scrolling"))
        {
            ...
            for (int i = 0; i < 5; i++)
            {
                ...
                ImGui::Text("%s",
                            i == 0
                                ? "Top"
                                : i == 1 ? "25%" : i == 2 ? "Center" : i == 3 ? "75%" : "Bottom");
    ...
    if (ImGui::CollapsingHeader("Popups & Modal windows"))
    {
        ...
        if (ImGui::TreeNode("Context menus"))
        {
            ...
            ImGui::Button(buf);
            if (ImGui::BeginPopupContextItem())  // When used after an item that has an ID (here the
                                                 // Button), we can skip providing an ID to
    ...
    if (ImGui::CollapsingHeader("Inputs, Navigation & Focus"))
    {
        ...
        ImGui::CheckboxFlags("io.ConfigFlags: NavEnableGamepad [beta]",
                             (unsigned int*) &io.ConfigFlags,
                             ImGuiConfigFlags_NavEnableGamepad);
        ImGui::CheckboxFlags("io.ConfigFlags: NavEnableKeyboard [beta]",
                             (unsigned int*) &io.ConfigFlags,
    ...
    if (ImGui::TreeNode("Colors"))
    {
        ...
        for (int i = 0; i < ImGuiCol_COUNT; i++)
        {
            ...
            if (memcmp(&style.Colors[i], &ref->Colors[i], sizeof(ImVec4)) != 0)
            {
                ...
                // the main font, so instead of "Save"/"Revert" you'd use icons. Read the FAQ and
                // misc/fonts/README.txt about using icon fonts. It's really easy and super
                // convenient!
    ...
    if (fonts_opened)
    {
        ...
        for (int i = 0; i < atlas->Fonts.Size; i++)
        {
            ...
            if (font_details_opened)
            {
                ...
                ImGui::Text(
                    "Fallback character: '%c' (%d)", font->FallbackChar, font->FallbackChar);
...
// This apply to regular items as well. Read FAQ section "How can I have multiple widgets with the
// same label? Can I have widget without a label? (Yes). A primer on the purpose of labels/IDs." for
// details.
...
static void ShowExampleAppCustomRendering(bool* p_open)
{
    ...
    ImGui::ColorEdit3("Color", &col.x);
    {
        ...
        for (int n = 0; n < 2; n++)
        {
            ...
            draw_list->AddCircle(
                ImVec2(x + sz * 0.5f, y + sz * 0.5f), sz * 0.5f, col32, 20, curr_thickness);
            x += sz + spacing;
...
struct ExampleAppConsole
{
    ...
    static int TextEditCallbackStub(
        ImGuiTextEditCallbackData* data)  // In C++11 you are better off using lambdas for this sort
